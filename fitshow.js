...
// Die Konsole rechnet ihre Strecke allein aus der Trittfrequenz, rund
// 0,37 km/h je rpm: 75 rpm sind auf Stufe 1 mit 25 W dieselben 28 km/h wie
// auf Stufe 9 mit 170 W. Die Fahrt vom 2026-09-25 kam so auf 12,3 km in
// 26 Minuten. Das Tempo folgt hier der Leistung wie draussen auf ebener
// Strasse: Rollwiderstand und Luft eines aufrecht sitzenden Fahrers.
export const FLAT = { mass: 100, crr: 0.005, cda: 0.4, rho: 1.2 };

/** Steady speed in m/s that `watts` hold on a flat road, 0 below 1 W. */
export function speedForPower(watts, { mass, crr, cda, rho } = FLAT) {
  if (!(watts > 1)) return 0;
  const roll = mass * 9.81 * crr, air = 0.5 * rho * cda;
  let lo = 0, hi = 30;
  for (let i = 0; i < 40; i++) {
    const v = (lo + hi) / 2;
    if (v * (roll + air * v * v) < watts) lo = v; else hi = v;
  }
  return lo;
}
